...
def main() -> int:
    parser = argparse.ArgumentParser(description="Test provider-specific gate decompositions")
    parser.add_argument("--backend", type=str, default=None, help="Backend name (auto-detect if not specified)")
    ...
    for pattern_name, circuit_fn in patterns.items():
        ...
        try:
            ...
            out_path = results_dir / f"pattern_{pattern_name}_{args.n_qubits}q_{backend_name.replace('.', '_')}_{ts}.json"
            out_path.write_text(json.dumps(out, indent=2), encoding="utf-8")

            unique = len(counts)
            print(f"  ✓ Results: {unique} unique states, entropy: {entropy:.3f}/{max_entropy:.3f}")
