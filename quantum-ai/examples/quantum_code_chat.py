...
-----
    ...
    python quantum-ai/examples/quantum_code_chat.py

    # Load existing checkpoint and chat immediately
    python quantum-ai/examples/quantum_code_chat.py --load

    # Train more before chatting
    python quantum-ai/examples/quantum_code_chat.py --epochs 20
...
def save_checkpoint(model: QuantumCodeLLM, tokenizer: CodeTokenizer) -> None:
    save_model_checkpoint(model, tokenizer, CHECKPOINT)
    print(f"  ✓ Saved -> {CHECKPOINT}")


def load_checkpoint() -> tuple[QuantumCodeLLM, CodeTokenizer]:
    model, tokenizer, _metadata = load_model_checkpoint(CHECKPOINT, map_location="cpu")
    print(f"  ✓ Loaded <- {CHECKPOINT}")
    return model, tokenizer


def retrain(model: QuantumCodeLLM, tokenizer: CodeTokenizer, n_epochs: int = 5) -> None:
    tcfg = TrainConfig(n_epochs=n_epochs, batch_size=8, lr=1e-3, log_every=40)
    trainer = QuantumCodeTrainer(model, tokenizer, tcfg)
    trainer.train()


def chat_loop(model: QuantumCodeLLM, tokenizer: CodeTokenizer) -> None:
    temperature = 0.8
    top_k = 40
    max_new = 80
