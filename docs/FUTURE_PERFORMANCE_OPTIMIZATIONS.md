...
- `generate_tags_fallback()` in web/aria_web/server.py
...
python -m cProfile -s cumulative web/aria_web/server.py > profile.txt
...
1. ✅ web/aria_web/server.py keyword sets (1.09x speedup)
2. ✅ shared/chat_memory.py connection pooling (50-100x for batch)
3. ✅ web/aria_web/server.py regex compilation (2-5x speedup)
