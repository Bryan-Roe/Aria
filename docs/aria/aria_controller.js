...
// GitHub Pages Demo Mode Configuration
...
async function mockApiCall(endpoint, options) {
    if (!DEMO_MODE) {
        // In non-demo mode, make real API call
    ...
    if (endpoint === '/api/aria/command') {
        ...
        return {
            ...
            json: async () => ({
                success: true,
                response: `Understood: "${body.command}". In demo mode, I'm simulating the response!`,
                tags: [],
                command: body.command
            })
        };
    } else if (endpoint === '/api/aria/object') {
        const body = JSON.parse(options.body);
        return {
            ok: true,
            status: 200,
    ...
    return {
        ok: true,
        status: 200,
        json: async () => ({ success: true, demo: true })
    };
}
...
async function sendCommand() {
    ...
    try {
        ...
        const response = await mockApiCall('/api/aria/command', {
            method: 'POST',
...
async function sendObjectUpdate(action, objectId, extra = {}) {
    try {
        ...
        const res = await mockApiCall('/api/aria/object', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
