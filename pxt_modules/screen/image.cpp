 ...
 */
//% property
int width(Image_ img) { return img->width(); }

/**
 * Get the height of the image
 */
//% property
int height(Image_ img) { return img->height(); }

/**
 * True if the image is monochromatic (black and white)
 */
//% property
bool isMono(Image_ img) { return img->bpp() == 1; }

//% property
bool isStatic(Image_ img) { return img->buffer->isReadOnly(); }

//% property
bool revision(Image_ img) { return img->revision; }

/**
 * Sets all pixels in the current image from the other image, which has to be of
 * the same size and bpp.
 */
//%
...
void fillRect(Image_ img, int x, int y, int w, int h, int c) {
  ...
  if (!img->hasPadding() && x == 0 && y == 0 && w == img->width() &&
      h == img->height()) {
    fill(img, c);
    return;
  }
...
void flipY(Image_ img) {
  ...
  // this is quite slow - for small 16x16 sprite it will take in the order of
  // 1ms something faster requires quite a bit of bit tweaking, especially for
  // mono images
...
void scroll(Image_ img, int dx, int dy) {
  ...
  if (dy != 0) {
    ...
    dx = -dx;
    if (dx < w)
      memmove(img->pix(), img->pix(dx, 0), static_cast<size_t>(w - dx) * static_cast<size_t>(bh));
    else
      dx = w;
...
//  byte 4...N: data 4 bits per pixels, high order nibble printed first, lines
//  aligned to 32 bit words byte 4...N: data 1 bit per pixels, high order bit
//  printed first, lines aligned to byte

Image_ convertAndWrap(Buffer buf) {
  if (isValidImage(buf))
    return NEW_GC(RefImage, buf);

  // What follows in this function is mostly dead code, except if people
  // construct image buffers by hand. Probably safe to remove in a year (middle
  // of 2020) or so. When removing, also remove from sim.
...
bool blit(Image_ dst, Image_ src, pxt::RefCollection *args) {
  ...
  for (int yDstCur = yDstStart, ySrcCur = ySrcStart;
       yDstCur < yDstEnd && ySrcCur < ySrcEnd; ++yDstCur, ySrcCur += ySrcStep) {
    int ySrcCurI = ySrcCur >> 16;
    for (int xDstCur = xDstStart, xSrcCur = xSrcStart;
         xDstCur < xDstEnd && xSrcCur < xSrcEnd;
...
inline int fxMul(int a, int b) { return (int)(((int64_t)a * b) >> FX_SHIFT); }

inline int fxDiv(int a, int b) { return (int)(((int64_t)a << FX_SHIFT) / b); }

inline int fxToInt(int v) { return v >> FX_SHIFT; }

inline int fxFloor(int v) { return v & 0xffff0000; }
...
ParsedShearArgs parseShearArgs(Image_ src, pxt::RefCollection *args,
                               int argIndex) {
...
void _drawScaledRotatedImage(Image_ dst, Image_ src, pxt::RefCollection *args) {
  ...
  if (shearArgs.sx <= 0 || shearArgs.sy <= 0 ||
    ...
    for (int y = 0; y < shearArgs.scaledHeight; y += FX_ONE) {
      for (int x = 0; x < shearArgs.scaledWidth; x += FX_ONE) {
        int color = getPixel(
            ...
            fxToInt(
                fxDiv((shearArgs.scaledHeight - y - FX_ONE), shearArgs.sy)));

        if (!color)
          continue;

        SHEAR(x, y, shearArgs.xShear, shearArgs.yShear);
        setPixel(dst, xDst + fxToInt(shearedX - shearArgs.minX),
                 yDst + fxToInt(shearedY - shearArgs.minY), color);
      }
    }
  } else {
    for (int y = 0; y < shearArgs.scaledHeight; y += FX_ONE) {
      for (int x = 0; x < shearArgs.scaledWidth; x += FX_ONE) {
        int color = getPixel(src, fxToInt(fxDiv(x, shearArgs.sx)),
...
//%
bool _checkOverlapsScaledRotatedImage(Image_ dst, Image_ src,
                                      pxt::RefCollection *args) {
  int xDst = pxt::toInt(args->getAt(0));
  int yDst = pxt::toInt(args->getAt(1));
  if (xDst >= dst->width() || yDst >= dst->height()) {
    return false;
  ...
  if (shearArgs.flip) {
    for (int y = 0; y < shearArgs.scaledHeight; y += FX_ONE) {
      for (int x = 0; x < shearArgs.scaledWidth; x += FX_ONE) {
        ...
        SHEAR(x, y, shearArgs.xShear, shearArgs.yShear);
        if (getPixel(dst, xDst + fxToInt(shearedX - shearArgs.minX),
                     yDst + fxToInt(shearedY - shearArgs.minY))) {
          return true;
        }
      }
    }
...
//%
bool _checkOverlapsTwoScaledRotatedImages(Image_ dst, Image_ src,
                                          pxt::RefCollection *args) {
  ...
  if (srcArgs.flip) {
    for (int y = 0; y < srcArgs.scaledHeight; y += FX_ONE) {
      for (int x = 0; x < srcArgs.scaledWidth; x += FX_ONE) {
        int color = getPixel(
            src, fxToInt(fxDiv((srcArgs.scaledWidth - x - FX_ONE), srcArgs.sx)),
        ...
        REVERSE_SHEAR(screenX + dstArgs.minX, screenY + dstArgs.minY,
                      dstArgs.xShear, dstArgs.yShear);

        if (dstArgs.flip) {
          if (getPixel(dst,
                       fxToInt(fxDiv(dstArgs.scaledWidth - unshearedX - FX_ONE,
                                     dstArgs.sx)),
                       fxToInt(fxDiv(dstArgs.scaledHeight - unshearedY - FX_ONE,
                                     dstArgs.sy)))) {
            return true;
          }
        } else if (getPixel(dst, fxToInt(fxDiv(unshearedX, dstArgs.sx)),
    ...
    for (int y = 0; y < srcArgs.scaledHeight; y += FX_ONE) {
      for (int x = 0; x < srcArgs.scaledWidth; x += FX_ONE) {
        int color = getPixel(src, fxToInt(fxDiv(x, srcArgs.sx)),
...
typedef struct {
  ...
  int D;
  int nextFuncIndex;
} LineGenState; // For keeping track of the state when generating Y values for a
                // line, even when moving to the next X.
...
// This function is similar to the sub-function drawLineHigh for drawLine.
// However, it yields back after calculating all Y values of a given X. When the
// function is called again, it continues from the state where it yielded back
// previously.
...
LineGenState initYRangeGenerator(int16_t X0, int16_t Y0, int16_t X1,
                                 int16_t Y1) {
  LineGenState line;
...
// core of draw vertical line for repeatly calling, eg.: fillTriangle() or
// fillPolygon4() value range/safety check not included prepare
// "img->makeWritable();" and "uint8_t f = img->fillMask(c);" outside required.
...
void fillTriangle(Image_ img, int x0, int y0, int x1, int y1, int x2, int y2,
                  int c) {
  if (x1 < x0) {
    swap(x0, x1);
    swap(y0, y1);
  ...
  // We have 3 different sub-functions to generate Ys of edges, each particular
  // edge maps to one of them. Use function pointers to avoid judging which
  // function to call at every X.
  typedef void (*FP_NEXT)(int x, LineGenState *line, ValueRange *yRange);
  FP_NEXT nextFuncList[] = {nextYRange_Low, nextYRange_HighUp,
                            nextYRange_HighDown};
...
void fillPolygon4(Image_ img, int x0, int y0, int x1, int y1, int x2, int y2,
                  int x3, int y3, int c) {
  LineGenState lines[] = {(x0 < x1) ? initYRangeGenerator(x0, y0, x1, y1)
                                    : initYRangeGenerator(x1, y1, x0, y0),
                          (x1 < x2) ? initYRangeGenerator(x1, y1, x2, y2)
                                    : initYRangeGenerator(x2, y2, x1, y1),
                          (x2 < x3) ? initYRangeGenerator(x2, y2, x3, y3)
                                    : initYRangeGenerator(x3, y3, x2, y2),
                          (x0 < x3) ? initYRangeGenerator(x0, y0, x3, y3)
                                    : initYRangeGenerator(x3, y3, x0, y0)};
...
void _fillTriangle(Image_ img, pxt::RefCollection *args) {
  fillTriangle(img, pxt::toInt(args->getAt(0)), pxt::toInt(args->getAt(1)),
               pxt::toInt(args->getAt(2)), pxt::toInt(args->getAt(3)),
               pxt::toInt(args->getAt(4)), pxt::toInt(args->getAt(5)),
               pxt::toInt(args->getAt(6)));
}

// This polygon fill is similar to fillTriangle(): Scan minY and maxY of all
// edges at each X, and draw a vertical line between (x,minY)~(x,maxY). The main
// difference is that it sorts the endpoints of each edge, x0 < x1, to draw from
// left to right, but doesn't sort the edges as it's too time consuming.
// Instead, just call next(), which returns immediately if the x is not in range
// of the edge in horizon. NOTE: Unlike triangles, edges of a polygon can cross
// a vertical line at a given X multi time. This algorithm can fill correctly
// only if edges meet this condition: Any vertical line(x) cross edges at most 2
// times. Fortunately, no matter what perspective transform is applied, a
// rectangle/trapezoid will still meet this condition. Ref:
...
void _fillPolygon4(Image_ img, pxt::RefCollection *args) {
  fillPolygon4(img, pxt::toInt(args->getAt(0)), pxt::toInt(args->getAt(1)),
               pxt::toInt(args->getAt(2)), pxt::toInt(args->getAt(3)),
               pxt::toInt(args->getAt(4)), pxt::toInt(args->getAt(5)),
               pxt::toInt(args->getAt(6)), pxt::toInt(args->getAt(7)),
 ...
 */
//%
Image_ ofBuffer(Buffer buf) { return ImageMethods::convertAndWrap(buf); }
...
extern "C" void *memcpy(void *dst, const void *src, size_t sz) {
  ...
  // see comment in memset() below (have not seen optimization here, but better
  // safe than sorry)
...
extern "C" void *memset(void *dst, int v, size_t sz) {
  ...
  // without volatile here, GCC may optimize the loop to memset() call which is
  // obviously not great
  volatile uint8_t *dd = (uint8_t *)dst;
