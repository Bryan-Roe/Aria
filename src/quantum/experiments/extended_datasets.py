...
from sklearn.datasets import make_classification, make_blobs, load_wine
...
scatter = ax1.scatter(X_xor[:, 0], X_xor[:, 1], c=y_xor, cmap='coolwarm', 
...
ax2.set_title(f'XOR Training (Final: {history_xor["val_acc"][-1]:.1%})', 
...
scatter = ax1.scatter(X_spiral[:, 0], X_spiral[:, 1], c=y_spiral, 
...
ax2.set_xlabel('Epoch', fontsize=12)
ax2.set_ylabel('Validation Accuracy', fontsize=12)
ax2.set_title(f'Spiral Training (Final: {history_spiral["val_acc"][-1]:.1%})', 
              fontsize=14, fontweight='bold')
ax2.grid(True, alpha=0.3)
...
for i, (dataset, history, color) in enumerate(zip(datasets, histories, colors)):
    ...
    axes[i].set_title(f'{dataset}: Acc={history["val_acc"][-1]:.3f}', 
