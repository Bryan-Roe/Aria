...
class PipelineOrchestrator:
    ...
    def step_gpu_optimization(self) -> Dict[str, Any]:
        ...
        cmd = [
            ...
            "--model-size", str(self.config.get("model_size_gb", 7.0)),
            "--memory-usage", str(self.config.get("memory_usage", 0.8)),
            "--output", "data_out/gpu_profile.yaml"
        ]
        
        if self.config.get("update_config", True):
            cmd.extend(["--update-config", self.config.get("training_config", "lora/lora.yaml")])
    ...
    def step_data_pruning(self) -> Dict[str, Any]:
        ...
        cmd = [
            ...
            "--input", input_path,
            "--output", output_path,
            "--similarity-threshold", str(self.config.get("similarity_threshold", 0.95)),
            "--quality-threshold", str(self.config.get("quality_threshold", 0.3))
        ]
    ...
    def step_training(self) -> Dict[str, Any]:
        ...
        cmd = [
            ...
            "--config", self.config.get("training_config", "lora/lora.yaml")
        ]
    ...
    def step_evaluation(self) -> Dict[str, Any]:
        ...
        cmd = [
            ...
            "--num-samples", str(self.config.get("eval_samples", 100)),
            "--output-name", self.config.get("experiment_name", "pipeline_eval")
        ]
...
Examples:
  ...
  python scripts/run_pipeline.py --input-dataset data/train.jsonl --test-dataset data/test.jsonl --rag-docs ../../datasets
    ...
    parser.add_argument("--training-config", type=str, default="lora/lora.yaml",
                        help="Training config file")
    parser.add_argument("--model-output", type=str, default="data_out/lora_training",
                        help="Output directory for trained model")
    
    # GPU Optimization
    parser.add_argument("--model-size-gb", type=float, default=7.0,
                        help="Model size in GB")
    parser.add_argument("--memory-usage", type=float, default=0.8,
    ...
    # Data Pruning
    parser.add_argument("--pruned-dataset", type=str, default="data/pruned_train.jsonl",
    ...
    parser.add_argument("--no-stream", action="store_true",
                        help="Disable streaming during training")
    ...
    parser.add_argument("--experiment-name", type=str, default="pipeline_eval",
                        help="Experiment name for results")
    
    # RAG
    parser.add_argument("--rag-docs", type=str,
                        help="Path to documents for RAG")
    parser.add_argument("--rag-index-path", type=str, default="data_out/rag_index",
                        help="Path for RAG index")
    parser.add_argument("--no-rebuild-rag-index", action="store_true",
                        help="Don't rebuild RAG index")
    
    # Pipeline Control
    parser.add_argument("--skip-optimization", action="store_true",
                        help="Skip GPU optimization step")
    parser.add_argument("--skip-pruning", action="store_true",
                        help="Skip data pruning step")
    parser.add_argument("--skip-training", action="store_true",
                        help="Skip training step")
    parser.add_argument("--skip-evaluation", action="store_true",
                        help="Skip evaluation step")
    parser.add_argument("--skip-rag", action="store_true",
                        help="Skip RAG setup step")
