...
def test_keyword_sets_are_frozen():
    ...
    from aria_web.server import _DANCE_KEYWORDS, _JUMP_KEYWORDS, _LIMB_KEYWORDS
...
def test_determine_position_performance():
    ...
    for cmd, expected in test_commands:
        ...
        assert (
            result == expected
        ), f"Command '{cmd}' returned {result}, expected {expected}"
...
def test_parse_command_performance():
    ...
    assert (
        elapsed < 0.05
    ), f"Command parsing too slow: {elapsed*1000:.2f}ms for 50 parses"
...
def test_connection_caching(mock_getenv, mock_pyodbc):
    ...
    from shared.chat_memory import _conn_cache, _conn_lock, _get_conn
...
def test_store_embedding_uses_cached_connection(mock_getenv, mock_pyodbc):
    ...
    from shared.chat_memory import _conn_cache, _conn_lock, store_embedding
...
def test_connection_pooling_speedup(mock_getenv, mock_pyodbc):
    ...
    print(
        f"✓ test_connection_pooling_speedup passed (10 operations in {elapsed*1000:.2f}ms)"
    )
