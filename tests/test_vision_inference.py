...
if missing:
    pytestmark = pytest.mark.skip(reason=f"Missing dependencies: {', '.join(missing)}")
else:
    import numpy as np
...
class TestVisionInference:
    ...
    def test_predict_file(
        self,
        dummy_checkpoint: tuple[Path, list[str]],
    ...
    def test_find_latest_checkpoint(
        ...
        with patch("scripts.vision_inference.Path") as mock_path:
            ...
            vi_instance = VisionInference.__new__(VisionInference)  # type: ignore
