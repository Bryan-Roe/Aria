...
def test_index_has_provider_and_model_controls() -> None:
    ...
    assert (
        'value="quantum-llm"' in html
    ), "Main Aria UI should include quantum provider option"
...
def test_controller_reads_override_inputs() -> None:
    ...
    assert (
        "getElementById('providerSelect')" in js
    ), "Controller should read provider selector"
    assert (
        "getElementById('modelInput')" in js
...
def test_controller_forwards_overrides_to_command_endpoint() -> None:
    ...
    assert (
        "provider: provider || undefined" in js
    ), "Controller payload should include provider override"
    assert (
        "model: model || undefined" in js
    ), "Controller payload should include model override"
...
def test_controller_persists_provider_settings_in_localstorage() -> None:
    ...
    assert (
        "restoreProviderSettings()" in js
    ), "Controller should restore provider settings on load"
    assert (
