...
def test_execute_payload_includes_provider_and_model() -> None:
    ...
    assert (
        "provider: provider || undefined" in html
    ), "Execute payload should forward provider override"
    assert (
        "model: model || undefined" in html
    ), "Execute payload should forward model/path override"
...
def test_results_show_requested_provider_and_model() -> None:
    ...
    assert (
        "Provider Requested:" in html
    ), "Results panel should display provider routing info"
    assert (
        "Model Requested:" in html
    ), "Results panel should display model/path override when provided"
...
def test_auto_execute_settings_persistence_wiring_present() -> None:
    ...
    assert (
        "aria-autoexec-provider" in html
    ), "Auto-execute should persist provider selection"
    ...
    assert (
        "aria-autoexec-show-state" in html
    ), "Auto-execute should persist show-state toggle"
    assert (
        "restoreAutoExecuteSettings()" in html
    ), "Auto-execute should restore saved settings on load"
    assert (
