...
class ParallelTrainer:
    ...
    def __init__(
        ...
        self.venv_python = self.root / "ai-projects/lora-training/microsoft_phi-silica-3.6_v1/venv/Scripts/python.exe"
        self.train_script = self.root / "ai-projects/lora-training/microsoft_phi-silica-3.6_v1/scripts/train_lora.py"
    ...
    async def run_job(self, job: Dict[str, Any], device_id: int) -> Dict[str, Any]:
        ...
        cmd = [
            ...
            self._validate_cmd_arg(
                job.get("config", "ai-projects/lora-training/microsoft_phi-silica-3.6_v1/lora/lora.yaml"),
                "config",
            ),
            "--dataset",
    ...
    async def run_all_parallel(self, job_filter: str = "*"):
        ...
        if skipped > 0:
            ...
            for r in skipped_jobs:
                print(
                    f"  - {r['name']}: {r.get('reason', 'min_train_samples threshold')} train_samples={r.get('dataset_train_samples')}"
                )
...
def main():
    ...
    parser.add_argument(
        "--filter",
        default="*",
        help="Job name filter pattern (e.g., 'phi35*' or 'quick*')",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Skip jobs whose counted train samples are below this threshold",
    )
    parser.add_argument(
        "--generate-samples",
        type=int,
        default=3,
        help="Number of sample generations to produce after successful training",
    )
    parser.add_argument(
        "--no-eval",
        action="store_true",
        help="Disable post-training evaluation and sample generation entirely",
    )
    parser.add_argument(
        "--cleanup",
        action="store_true",
        help="Remove intermediate checkpoint artifacts after successful training",
    )
    parser.add_argument(
        "--ranking-metric",
        choices=[
        ...
        help="Metric used to rank jobs in status history (distinct_diversity alias of diversity_avg)",
    )
