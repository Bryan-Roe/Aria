...
class NotificationManager:
    ...
    def _send_linux(self, title: str, message: str, icon: str):
        ...
        try:
            result = subprocess.run(
                ['notify-send', '-i', icon_name, title, message], capture_output=True, text=True, timeout=5)
            if result.returncode != 0:
                print(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--job-name', default='Test Job',
                        help='Job name for monitoring')
