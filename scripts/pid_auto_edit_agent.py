...
def run_loop(interval: int = 5) -> int:
    ...
    while True:
        ...
        task = QueueTask(
            ...
            model=(str(task_raw.get("model")) if task_raw.get(
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="PID-managed autonomous file-editing agent")
    sub = p.add_subparsers(dest="cmd", required=True)

    p_start = sub.add_parser("start", help="start daemon worker")
    p_start.add_argument("--interval", type=int, default=5,
    ...
    p_enqueue.add_argument("--task", required=True,
                           help="task description for autonomous_code_agent")
    p_enqueue.add_argument("--llm-type", default="echo",
                           choices=["echo", "ollama", "lmstudio"])
