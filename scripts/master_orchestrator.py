...
class MasterOrchestrator:
    ...
    def _match_cron_field(
        ...
        *,
        ...
        for part in field.split(","):
            ...
            # Handle step expressions (e.g., */15, 1-10/2)
            if "/" in part:
    ...
    def _should_run_now(self, workflow_name: str, schedule: str) -> bool:
        """Check if a cron-like schedule matches current UTC time.

        Uses standard 5-field cron expressions: minute hour day month day_of_week.
        Day-of-month and day-of-week follow OR semantics when both are constrained.
        """
        schedule = (schedule or "").strip()
        if not schedule or schedule.lower() == "continuous":
            return False
        ...
        minute_ok = self._match_cron_field(minute_f, now.minute, 0, 59)
        hour_ok = self._match_cron_field(hour_f, now.hour, 0, 23)
        month_ok = self._match_cron_field(month_f, now.month, 1, 12)
        if not (minute_ok and hour_ok and month_ok):
            return False

        # Cron day-of-week convention: 0 or 7 = Sunday, 1 = Monday, ... 6 = Saturday
        current_dow = (now.weekday() + 1) % 7
        dom_ok = self._match_cron_field(dom_f, now.day, 1, 31)
        dow_ok = self._match_cron_field(
            dow_f,
            current_dow,
            0,
            7,
        ...
        # Avoid duplicate triggers in the same minute when check_interval < 60.
        dedupe_key = f"{workflow_name}:{schedule}"
...
def main():
    ...
    # Validate configuration unless --skip-validation is set
    if not args.skip_validation:
        try:
            ...
            try:
                ...
                print("⚠️  config_validator not available, skipping validation")
        ...
        if ConfigValidator:
            ...
            if not result.valid:
                ...
                sys.exit(1)
            print(f"\n✅ Config validation passed: {config_path}")

    master = MasterOrchestrator(config_path)
