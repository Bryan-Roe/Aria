...
def _create_patched_from_agent_framework(original_func: Callable) -> Callable:
    ...
    def patched_from_agent_framework(agent: Any, **kwargs) -> Any:
        ...
        # Use an in-memory session repository for local development so the
        # Agent Server SDK doesn't try to reach the Foundry cloud service
        # (which would fail with "Malformed identifier" for locally-generated
        # conversation IDs).
        if 'session_repository' not in kwargs or kwargs['session_repository'] is None:
            try:
                from azure.ai.agentserver.agentframework.persistence import InMemoryAgentSessionRepository
                kwargs['session_repository'] = InMemoryAgentSessionRepository()
                if verbose:
                    print(f"agentdev Hook: Using InMemoryAgentSessionRepository for local development", file=sys.stderr)
            except ImportError:
                if verbose:
                    print(f"agentdev Hook: InMemoryAgentSessionRepository not available, using default", file=sys.stderr)
