...
class MCPClient:
    ...
    async def _register_server(self, server_id: str, session: ClientSession):
        ...
        self._servers[server_id] = {"session": session, "tools": tools}
        ...
        print(
            f"\nConnected to server '{server_id}' with tools:",
...
async def main():
    ...
    messages = [
        {
            "role": "developer",
            "content": 'Unable to inference the GitHub model gpt-4.1 due to 403.  To reduce such errors, use [GitHub pay-as-you-go models](command:ai-mlstudio.internal.GHModelsDoc) or [Deploy to Azure AI Foundry](command:ai-mlstudio.internal.triggerFoundryModelDeployment?["gpt-4.1","github403"]).',
        },
        UserMessage(
            content=[
