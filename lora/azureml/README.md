...
```powershell
az ml job create -f lora/azureml/job-lora-train.yml --set inputs.max_train_samples=256 inputs.max_eval_samples=64 compute=azureml:gpu-cluster
```
...
```powershell
az ml job create -f lora/azureml/job-lora-sweep.yml --set compute=azureml:gpu-cluster
```
Monitor trials:
```powershell
