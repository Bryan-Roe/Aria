...
def _trainer_tokenizer_kwarg() -> str:
    """Return the keyword name to pass a tokenizer to ``transformers.Trainer``.

    Transformers versions prior to 5 accepted ``tokenizer``; starting with
    v5 the argument was renamed to ``processing_class``.  We inspect the
    signature at runtime so callers (and tests) can determine the correct
    keyword without hardcoding a version check.

    Returns
    -------
    str
        Either "tokenizer" or "processing_class" depending on what the
        installed ``transformers.Trainer`` expects.
    """
    import inspect
    ...
    if "tokenizer" in params:
        ...
        # new names used in transformers>=5
...
def main():
    ...
    # build trainer kwargs dynamically so we don't blow up when the
    # transformers version changes
    tk_arg = _trainer_tokenizer_kwarg()
    trainer_kwargs = {
        "model": model,
        "args": training_args,
    ...
    if TrainerCallback is not None:
        ...
        try:
            # Always attempt to add the OpenTelemetry callback.  The
            # implementation is intentionally defensive and swallows
            # exceptions, so it won't crash training even if certain
            # hook methods are missing.  This is simpler than trying to
            # probe the class for specific attributes, which proved
            # fragile in the wild when the callback was redefined.
