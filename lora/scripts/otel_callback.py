...
class OpenTelemetryTrainerCallback:
    ...
    # The Hugging Face Trainer may call ``on_epoch_begin`` as part of its
    # callback lifecycle.  Older versions of this module did not implement
    # that hook and the absence caused training to crash when the trainer
    # attempted to invoke it.  We include a no-op implementation here so
    # that the callback is always safe to add regardless of HF version or
    # which events the user cares about.
    def on_epoch_begin(self, args, state, control, **kwargs):
        # no telemetry for individual epochs at the moment
        return
