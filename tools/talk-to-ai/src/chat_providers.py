...
# Backward-compatibility aliases used by older modules and unit tests.  The
# former implementation kept the cache in `_lmstudio_cache` with a separate
# TTL constant; we now point those names at the canonical objects so that
# manipulating one affects the other.
_lmstudio_cache = _lm_studio_availability_cache  # type: ignore
