...
class AzureMLLoRATrainer:
    ...
    def create_environment(
        ...
        env = Environment(
            ...
            image=f"mcr.microsoft.com/azureml/curated/acft-hf-nlp-gpu:63",
            conda_file={
                "name": env_name,
    ...
    def submit_training_job(
        ...
        args = [
            "--config", config_path,
            "--dataset", Input(type=AssetTypes.URI_FOLDER, path=f"azureml:{dataset_name}@latest"),
...
def main():
    ...
    # Actions
    parser.add_argument("--action", choices=["setup", "upload", "train", "register"], 
                       default="train", help="Action to perform")
    
    # Training settings
    parser.add_argument("--experiment-name", default="phi36-lora-training", 
                       help="Experiment name")
    parser.add_argument("--compute-name", default="phi36-gpu-cluster",
    ...
    parser.add_argument("--dataset-path", default="../../datasets/chat/dolly",
                       help="Local dataset path")
    parser.add_argument("--dataset-name", default="dolly-chat-dataset",
                       help="Dataset name in Azure ML")
    parser.add_argument("--config", default="./lora/lora.yaml",
                       help="Path to lora.yaml config")
    parser.add_argument("--max-train-samples", type=int, default=None,
                       help="Limit training samples (for testing)")
    
    # Model registration
    parser.add_argument("--job-name", help="Job name for model registration")
    parser.add_argument("--model-name", default="phi36-lora-adapter",
