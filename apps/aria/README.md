...
> **For AI agents:** discover the full action contract at runtime via
> `GET /api/aria/schema`, and example commands via `GET /api/aria/presets`.
> An offline copy of the schema is committed at
> [`action_schema.json`](./action_schema.json) and regenerated by
> `python scripts/generate_aria_schema.py`. See [`AGENTS.md`](../../AGENTS.md)
> at the repo root for the AI-oriented quick reference.
