...
> Note: `QAI_PROVIDER_PRIORITY` in `shared/config.py` is a separate _declared_ priority list (default `["azure", "openai", "lmstudio", "local"]`) surfaced by `Settings.provider_chain()`; it is distinct from the runtime `detect_provider()` probe order above.
