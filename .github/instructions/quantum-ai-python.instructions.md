...
- Prefer local simulation first (Qiskit Aer) before Azure hardware.
  - Dry-run orchestrator: `.venv/bin/python scripts/quantum_autorun.py --dry-run`
  - Simulator job: `.venv/bin/python scripts/quantum_autorun.py --job azure_ionq_simulator`
- Cost gate: set `azure_confirm_cost: true` in `quantum_autorun.yaml` before real QPU; start with ≤100 shots.
- MCP server for tooling: `.venv/bin/python ai-projects/quantum-ml/quantum_mcp_server.py`
...
- Tests: `.venv/bin/python scripts/test_runner.py --unit` or `pytest -m "not slow and not azure"`.
